...
describe("SituationDescription", () => {
  ...
  test("constructor - rejects under the limit", () => {
    expect(() => new SituationDescription("")).toThrow(
  ...
  test("constructor - rejects over the limit", () => {
    expect(
      () =>
        new SituationDescription(
          "a".repeat(SituationDescription.MaximumLength + 1),
        ),
    ).toThrow(SituationDescription.Errors.invalid);
  });
