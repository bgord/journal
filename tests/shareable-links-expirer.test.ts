...
describe("WeeklyReviewScheduler", () => {
  test("validation - ShareableLinkIsActive - already revoked", async () => {
    ...
    spyOn(Publishing.Repos.ShareableLinkRepository, "listExpired").mockResolvedValue([mocks.shareableLink]);
  ...
  test("repository failure", async () => {
    spyOn(tools.Revision.prototype, "next").mockImplementation(() => mocks.revision);
    spyOn(Publishing.Repos.ShareableLinkRepository, "listExpired").mockImplementation(() => {
  ...
  test("correct path - no links", async () => {
    spyOn(Publishing.Repos.ShareableLinkRepository, "listExpired").mockResolvedValue([]);
    const eventStoreSave = spyOn(EventStore, "save").mockImplementation(jest.fn());
