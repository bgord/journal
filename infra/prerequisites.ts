...
export function createPrerequisites(Env: EnvironmentType, deps: Dependencies) {
  ...
  return [
    ...
    new bg.PrerequisiteLogFile({ label: "log-file", enabled }, deps),
    ...
    new bg.PrerequisiteMailer({ label: "mailer", enabled }, deps),
    ...
    new bg.PrerequisiteSSLCertificateExpiry(
      { label: "ssl", hostname: "journal.bgord.dev", days: 7, enabled },
      deps,
    ),
    new bg.PrerequisiteClockDrift({ label: "clock-drift", enabled, skew: tools.Duration.Minutes(1) }, deps),
