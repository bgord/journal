...
import { ReactPdfGenerator } from "./react-pdf-generator.adapter";
...
class NoopPdfGenerator implements Ports.PdfGeneratorPort {
  async request(template: Ports.PdfGeneratorTemplateType, data: Record<string, unknown>): Promise<Buffer> {
    logger.info({
...
export const PdfGenerator =
  Env.type === bg.NodeEnvironmentEnum.local ? new ReactPdfGenerator() : new NoopPdfGenerator();
